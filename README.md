...
- **生产加固建议**：
  1. 首次配置完成后**删除 `setup-key.txt`**（只用于第一次初始化）；
  2. 在 `.env` 里**显式设置独立的 `MCP_JWT_SECRET`**（`openssl rand -hex 32`）与 `MCP_DB_ENC_KEY`，避免会话密钥/字段加密密钥都从 SETUP_KEY 派生（SETUP_KEY 一旦泄露即影响全部）；
